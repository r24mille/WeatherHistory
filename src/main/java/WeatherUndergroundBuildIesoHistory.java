...
public class WeatherUndergroundBuildIesoHistory {
	...
	static final String city = "Windsor";
		...
		try {
			HistoryResponse historyResponse = restTemplate.getForObject(
					"http://api.wunderground.com/api/" + apiKey + "/history_"
							+ dateString + "/q/" + country + "/" + cityForURL
							+ ".json", HistoryResponse.class);
			System.out
					.println("Request URL: "
							+ "http://api.wunderground.com/api/" + apiKey
							+ "/history_" + dateString + "/q/" + country + "/"
							+ cityForURL + ".json");
			for (Observation observation : historyResponse.getHistory()
				...
				/*
				 * METAR observations are scheduled observations
				 * http://www.wunderground.com/metarFAQ.asp and should prevent
				 * duplicates on the hour from separate stations and unneeded
				 * inter-hour observations.
				 */
